/**
 * Direct VSR Governance Power Calculator
 * Uses direct RPC calls and proper VSR struct parsing
 * No Anchor dependencies - pure blockchain data extraction
 */
...
/**
 * Find all VSR Voter accounts for a wallet
 */
async function findAllVoterAccounts(walletPubkey) {
  console.log(`    Searching for VSR accounts for ${walletPubkey.toBase58().substring(0, 8)}...`);
  
  const accounts = [];
  
  // Search at different authority offsets
...
function parseVoterAccount(data, accountAddress) {
  ...
  // Parse deposit entries starting at offset 80
 ...
 */
async function calculateGovernancePowerDirect(walletAddress) {
  try {
    const walletPubkey = new PublicKey(walletAddress);
    
    const voterAccounts = await findAllVoterAccounts(walletPubkey);
    ...
    for (const accountInfo of voterAccounts) {
      ...
      const deposits = parseVoterAccount(accountInfo.account.data, accountAddress);
...
/**
 * Process all citizens with direct VSR calculation
 */
async function processAllCitizensDirect() {
  console.log('=== Direct VSR Governance Calculator ===');
  console.log('Uses direct RPC calls and VSR struct parsing');
  console.log('Pure blockchain data with no external dependencies');
  console.log('');
  ...
  for (let i = 0; i < citizens.length; i++) {
    ...
    const { totalPower, deposits, accounts } = await calculateGovernancePowerDirect(citizen.wallet);
  ...
  if (validationsFailed === 0) {
    ...
    console.log('Current blockchain data shows different values than expected targets');
  }
  ...
  if (validationsFailed === 0) {
    console.log('\n🎯 SUCCESS: All validations passed - this matches authentic blockchain data');
  } else {
    console.log('\n📊 COMPLETE: Processed all VSR accounts - showing authentic on-chain governance power');
  }
...
if (require.main === module) {
  processAllCitizensDirect().catch((error) => {
...
module.exports = {
  processAllCitizensDirect,
  calculateGovernancePowerDirect
};