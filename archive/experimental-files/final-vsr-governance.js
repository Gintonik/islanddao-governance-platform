 ...
 * Uses the most precise validation to avoid counting invalid deposits
 * Only counts deposits that are both reasonable amounts AND have proper activation patterns
 */
...
/**
 * Parse VSR deposits with precise validation
 * Filters out timestamps and other data misread as deposits
 */
function parseVSRDepositsStrict(data, accountAddress) {
  ...
  // Scan for deposit amounts with strict validation
  for (let offset = 0; offset < data.length - 16; offset += 8) {
    try {
      ...
      // Strict amount validation - exclude obvious timestamps/non-deposits
      ...
      // Look for dedicated activation flag (value = 1) at specific offset distance
      ...
      for (const distance of flagDistances) {
        ...
        if (checkOffset + 8 <= data.length) {
          try {
            ...
            if (flagValue === 1) {
              // Additional validation: ensure this flag doesn't correspond to another deposit
...
async function processAllCitizens() {
  ...
  console.log('Using strict validation to exclude timestamp/invalid deposits');
  console.log('');
  ...
  for (let i = 0; i < citizens.length; i++) {
    ...
    if (deposits.length > 0) {
      ...
      for (const deposit of deposits) {
        ...
        console.log(`      Data @ offset ${deposit.offset}, flag @ ${deposit.flagOffset}`);
      }
    ...
    // Critical validation for key wallets
    if (citizen.wallet === 'Fgv1zrwB6VF3jc45PaNT5t9AnSsJrwb8r7aMNip5fRY1') {
      if (Math.abs(totalPower - 200000) < 1) {
        ...
        console.log(`  ⚠️  CRITICAL: Must show exactly 200,000 ISLAND`);
      }
    }
  ...
  console.log('\n✅ Final VSR governance power calculation completed');
  console.log('Strict validation applied - timestamps and invalid deposits excluded');
