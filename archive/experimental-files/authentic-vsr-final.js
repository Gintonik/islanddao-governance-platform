 ...
 * Uses real IslandDAO registrar config with proper scale conversion (10^18)
 * Applies authentic VSR multiplier logic for Constant lockups only
 */
    ...
    for (let offset = 0; offset < data.length - 80; offset += 4) {
      try {
        ...
        if (mint.equals(ISLAND_MINT)) {
          ...
          // Read baseline_vote_weight.scale (u64)
          const baselineVoteWeightScale = Number(data.readBigUInt64LE(configOffset));
          configOffset += 8;
          
          // Read max_extra_lockup_vote_weight.scale (u64)
          const maxExtraLockupVoteWeightScale = Number(data.readBigUInt64LE(configOffset));
          configOffset += 8;
          ...
          if (baselineVoteWeightScale > 0 && maxExtraLockupVoteWeightScale > 0 && lockupSaturationSecs > 0) {
            // Convert from fixed-point format (scale = 10^18) to float
            const baselineVoteWeight = baselineVoteWeightScale / 1e18;
            const maxExtraLockupVoteWeight = maxExtraLockupVoteWeightScale / 1e18;
            ...
            const config = {
              ...
              rawBaselineScale: baselineVoteWeightScale,
              rawMaxExtraScale: maxExtraLockupVoteWeightScale
            };
            
            console.log('✓ AUTHENTIC VSR CONFIG EXTRACTED:');
            console.log(`  baseline_vote_weight.scale: ${baselineVoteWeightScale} (${baselineVoteWeight} float)`);
            console.log(`  max_extra_lockup_vote_weight.scale: ${maxExtraLockupVoteWeightScale} (${maxExtraLockupVoteWeight} float)`);
...
/**
 * Calculate multiplier using authentic VSR logic with proper scale conversion
 */
function calculateAuthenticMultiplier(deposit) {
  const currentTime = Math.floor(Date.now() / 1000);
  
  // Use the authentic values from registrar (already converted from 10^18 scale)
...
async function run() {
  ...
  console.log('Using real IslandDAO registrar config with proper scale conversion (10^18)');
  console.log('Applying authentic VSR multiplier logic for Constant lockups only');
  console.log('');
  ...
  try {
    ...
    try {
      ...
      if (citizenResults.length > 0) {
        console.log('\n=== DETAILED BREAKDOWN PER CITIZEN ===');
