 ...
 * Uses authentic IslandDAO registrar values with proper multiplier formula
 * Applies governance power calculation to all deposits based on lockup status
 */
...
// Global registrar config (fetched from authentic blockchain data)
let registrarConfig = null;

/**
 * Extract authentic VSR configuration from the known registrar account
 */
async function fetchAuthenticRegistrarConfig() {
  try {
    ...
    // Search for ISLAND mint in the data and extract config values
    for (let offset = 0; offset < data.length - 60; offset += 4) {
      try {
        ...
        if (potentialMint.equals(ISLAND_MINT)) {
          ...
          // Extract the three config values after the mint
          ...
          // Apply proper scaling - these appear to use 1e18 scale
          const baselineVoteWeight = rawBaseline / 1e18;
          const maxExtraLockupVoteWeight = rawMaxExtra / 1e18;
          const lockupSaturationSecs = rawSaturation;
    ...
    throw new Error('Could not extract VSR configuration from registrar account');
...
/**
 * Calculate multiplier using authentic VSR formula from registrar values
 */
...
async function run() {
  ...
  console.log('Uses authentic IslandDAO registrar values with proper multiplier formula');
  console.log('Applies governance power calculation to all deposits based on lockup status');
  console.log('');
