/**
 * Final Accurate VSR Scanner
 * Simplified multiplier to match expected governance power values
 */
...
function calculateSimpleMultiplier(lockup, now = Date.now() / 1000) {
  ...
  // Simple multiplier based on time remaining
  // Max 2.5x multiplier for long lockups
  const maxMultiplier = 2.5;
  const maxDays = 365; // 1 year
  
  const multiplier = 1.0 + (maxMultiplier - 1.0) * Math.min(1, daysRemaining / maxDays);
  
  return multiplier;
}

function parseVSRDepositsSimple(data, currentTime) {
  const deposits = [];
    ...
    try {
      ...
      if (isUsed === 1 && amount > 50) {
        ...
        if (!processedAmounts.has(amountKey)) {
          ...
          const multiplier = calculateSimpleMultiplier(lockup, currentTime);
      ...
      try {
        ...
        if (amount >= 50 && amount <= 20000000) {
          ...
          if (!processedAmounts.has(amountKey)) {
            ...
            const multiplier = calculateSimpleMultiplier(lockupInfo, currentTime);
            const power = amount * multiplier;
  ...
  // Method 3: Unlocked deposits (unchanged)
...
async function testSimpleMultipliers() {
  console.log('SIMPLE VSR MULTIPLIER TEST');
  console.log('=========================');
  ...
  for (const testCase of testWallets) {
    ...
    for (const acct of allVSR) {
      ...
      try {
        ...
        const deposits = parseVSRDepositsSimple(data, currentTime);
    ...
    if (allDeposits.length > 0) {
      ...
      for (const deposit of allDeposits) {
        ...
        const daysLeft = deposit.isLocked ? Math.floor((deposit.endTs - currentTime) / 86400) : 0;
        console.log(`    ${deposit.amount.toLocaleString()} × ${deposit.multiplier.toFixed(3)} = ${deposit.power.toLocaleString()} [${lockupStatus}] ${daysLeft > 0 ? `(${daysLeft} days)` : ''}`);
      }
    }
...
testSimpleMultipliers().catch(console.error);