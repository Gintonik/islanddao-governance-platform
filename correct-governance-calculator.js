/**
 * Correct Governance Calculator
 * Uses authentic VSR values that match the governance interface
 * Takes the primary governance power value from VSR accounts
 */
...
/**
 * Extract native governance power using the correct methodology
 * Use only the highest governance power value found across all VSR accounts
 */
async function getNativeGovernancePower(walletAddress) {
  ...
  for (const account of vsrAccounts) {
    try {
      ...
      // Check both governance power offsets and take the maximum
          ...
          try {
            ...
            if (value > 1000 && value < 50000000) {
              if (value > maxGovernancePower) {
                ...
                if (accountsFound <= 3) {
                  console.log(`    Found governance power: ${value.toLocaleString()} ISLAND (offset ${offset})`);
                }
              }
            }
          } catch (error) {
  ...
  if (accountsFound > 0) {
    console.log(`  Found ${accountsFound} VSR accounts, max power: ${maxGovernancePower.toLocaleString()} ISLAND`);
  }
...
/**
 * Update all citizens with correct governance calculation
 */
async function updateAllCitizensCorrectGovernance() {
  try {
    console.log('🔄 Starting correct governance calculation for all citizens...');
    ...
    console.log('✅ Correct governance calculation completed');
    ...
    console.error('❌ Error in correct governance calculation:', error);
    throw error;
  }
}
...
module.exports = {
  ...
  updateAllCitizensCorrectGovernance
};

// Run calculation when called directly
if (require.main === module) {
  updateAllCitizensCorrectGovernance().catch(console.error);
}