 ...
 * Properly sums governance power from ALL VSR accounts for each wallet
 * Handles multiple accounts per wallet with different governance power values
 */
 ...
 * Sums governance power from all accounts containing the wallet
 */
async function getNativeGovernancePower(walletAddress) {
  ...
  for (const account of vsrAccounts) {
    try {
      ...
      // Extract governance power from all relevant offsets
      // Based on analysis: offset 104 and 112 contain governance power values
        ...
        if (offset + 8 <= data.length) {
          try {
            ...
            if (value > 1000 && value < 50000000) {
              totalGovernancePower += value;
              
              if (accountsFound <= 3) { // Log details for first few accounts
                console.log(`    Account ${accountsFound}: +${value.toLocaleString()} ISLAND (offset ${offset})`);
              }
            }
          } catch (error) {
            // Skip invalid data
...
/**
 * Update all citizens with corrected governance calculation
 */
async function updateAllCitizensCorrectedGovernance() {
  try {
    console.log('🔄 Starting corrected governance calculation for all citizens...');
    ...
    console.log('✅ Corrected governance calculation completed');
    ...
    console.error('❌ Error in corrected governance calculation:', error);
    throw error;
  }
}
...
module.exports = {
  ...
  updateAllCitizensCorrectedGovernance
};

// Run calculation when called directly
if (require.main === module) {
  updateAllCitizensCorrectedGovernance().catch(console.error);
}